...
setwd(paste0("C:/Users/ciana/OneDrive/SONY_16M1/Football Predictions/Europe/",
      "Output Data"))
# get yourself a handful of data
...
#---------------- Gradient Boosting (Create prediction data) -----------------#
		# The below line looks like a stupid mistake on my part but actually 
        # it's a deliberate mistake to counter a stupid mistake on R's part
...
#---------------- Gradient Boosting (Hyperparameter Tuning) ------------------#
        ...
        r = resample(lrn, trainTask, resampling = outer,
...
#--------------- Gradient Boosting (select best parameters) ------------------#
		...
		# Transform Traindat back to being just numeric for the actual model 
        # build
...
#---------------- Gradient Boosting (Find optimal c-values) ------------------#
        ...
        # and checks what the story is with the old accuracy levels, for each 
        # combination we want to figure out what the maximum accuracy achieved 
        # is and then we want to see where the elbow point is for the pc's 
        # we'll get that by looking at where there's the greatest increase in 
        # predictions and take the first one

		a <- nrow(Resers)
		b <- ncol(Resers)

		# Create ResP1 which is the actual goal difference transformed in to 
        # class results (-1,0,1) first column of our new dataframe will be 
        # the actual result
        ResP1 <- data.frame()
		...
		# so for each of the possible C-value combinations we want to check 
        # what the accuracy is ToSt is to house the
		ToSt <- as.data.frame(1)
		for (d in 1:nrow(CValues)){

			for(f in 2:ncol(Resers)){
				# for each principle component we classify the predicted values 
                # in to what the result would be
                ResP1[, f] <- ifelse(Resers[, f] > CValues[d, 1], 1, ifelse(
                                     Resers[,f] < -1*CValues[d,2], -1, 0))
				}
			# Sust houses the count of correct predictions
			# first we set it to be a series of 1's so it's the right size, as
            # above
            SuSt <- rep(1,a)
	...
	# rename the matrix that has the different results based on all the c-value
